...
def compute_hvp(model: nn.Module, loss_fn: nn.Module, batch: Tuple, vec: Tensor, P: Tensor = None):
    # hvp = P^{-1} H v <--> P^{-1/2} H P^{-1/2} v 
    
    x, y = batch # extract the batch
...
def power_iteration(model, loss_fn, batch, vec: Tensor = None, max_iters: int = 200, tol: float = 1e-06, P = None):
    ...
    if vec is None: # initialize a random vector
        vec = torch.randn(nparams, dtype = torch.float32, requires_grad = False).to(device)
...
def get_presharpness_power_method(model: nn.Module, loss_fn: nn.Module, optim: torch.optim.Optimizer, batch: Tuple, max_iters: int = 10_000, tol: float = 1e-06, vec: Tensor = None):
