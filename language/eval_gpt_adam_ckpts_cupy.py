...
def estimate_critical_lr(
    ctx, model, loss_fn, optim, eval_steps, data_dir, context_len, batch_size, device
):
    lrs_mean = {}
    lrs_std = {}
    steps = {}
...
def estimate_sharpness(
    ...
    for k in range(eval_steps):
        ...
        pre_sharpness, pre_eigvec, num_iters_pre_sharpness = (
            sharpness_cupy_utils.get_pre_sharpness_lobpcg(
                model,
                loss_fn,
                optim,
                (X, Y),
        ...
        print(
            f"Pre-sharpness: {pre_sharpness:.4e}, num_iters_pre_sharpness: {num_iters_pre_sharpness}"
        )
        ...
        # estimate sharpness
        sharpness, eigvec, num_iters_sharpness = (
            sharpness_cupy_utils.get_sharpness_lobpcg(
                model,
                loss_fn,
                (X, Y),
                eigvecs=eigvec,
...
def evaluate_ckpts_on_datasets(cfg, device):
    ...
    ckpt_steps_lst = [
        cfg.ckpt_interval * i for i in range(0, cfg.num_steps // cfg.ckpt_interval + 1)
    ]
    ...
    eval_results = DataStorage(
        columns=["ckpt_step", "lr_step", "train_loss", "val_loss", "dataset"]
    )
    ...
    forward_results = DataStorage(
        columns=[
            "ckpt_step",
            "lr_step",
            "critical_lr_train_mean",
            "critical_lr_train_std",
            "critical_lr_val_mean",
            "critical_lr_val_std",
            "num_iters_train",
            "num_iters_val",
            "dataset",
    ...
    for ckpt_steps in ckpt_steps_lst:
        ...
        model, loss_fn, train_optim = create_train_state(
            cfg, device, ckpt_steps
        )  # move model to device and compile it later
        ...
        print(
            f"Loaded checkpoint at step {ckpt_steps} with {num_params} parameters, {embd_params} embedding parameters"
        )
        ...
        for dset in datasets:
            ...
            optim = (
                train_optim if dset == "fineweb" else downstream_optim
            )  # use the dummy optimizer if recomputing gradients is not needed
            ...
            )  # get a batch for sharpness estimation
            ...
            print(
                f"Losses on ckpt at step: {ckpt_steps} on dataset: {dset} - train: {losses['train'].item():.4f}, val: {losses['val'].item():.4f}"
            )

            # estimate critical learning rate
            lrs_mean, lrs_std, iters = estimate_critical_lr(
                ctx,
                model,
                loss_fn,
                optim,
                cfg.forward_eval_steps,
                data_dir,
                cfg.context_len,
                cfg.batch_size,
                device,
            ...
            forward_results.add_entry(
                ckpt_step=ckpt_steps,
                lr_step=lr_step,
                critical_lr_train_mean=lrs_mean["train"],
                critical_lr_train_std=lrs_std["train"],
                critical_lr_val_mean=lrs_mean["val"],
                critical_lr_val_std=lrs_std["val"],
                num_iters_train=iters["train"],
                num_iters_val=iters["val"],
                dataset=dset,
            )
            ...
            print(
                f"Critical learning rate on ckpt at step: {ckpt_steps} on dataset: {dset} - train: {lrs_mean['train']:.2e}, val: {lrs_mean['val']:.2e}, num_iters_train: {iters['train']}, num_iters_val: {iters['val']}"
            )
            ...
            (
                ...
                loss_fn,
                optim,
                cfg.sharpness_eval_steps,
                data_dir,
                cfg.context_len,
                cfg.batch_size,
                device,
                max_iters=cfg.max_iters,
                sharpness_tol=cfg.sharpness_tol,
            )
            sharpness_results.add_entry(
                ckpt_step=ckpt_steps,
                lr_step=lr_step,
                pre_sharpness=pre_sharpness,
                pre_sharpness_std=pre_sharpness_std,
                pre_sharpnes_converged=pre_sharpness_converged,
                sharpness=sharpness,
                sharpness_std=sharpness_std,
                sharpness_converged=sharpness_converged,
                dataset=dset,
            )
            ...
            print(
                f"Sharpness on ckpt at step: {ckpt_steps} on dataset: {dset} - pre-sharpness: {pre_sharpness:.4e}, pre_sharpness_std {pre_sharpness_std:0.4f}, pre-sharpness converged: {pre_sharpness_converged}, sharpness: {sharpness:.4e}, sharpness_std: {sharpness_std:0.4f}, sharpness converged: {sharpness_converged}"
            )
