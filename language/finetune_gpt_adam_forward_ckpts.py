...
def extract_model_properties(filename):
    patterns = [
        r'(fineweb)',                  # fineweb
        r'(gpt)',                      # gpt (no var1.0)
        r'(d\d+)',                     # d12
...
def get_base_filename(cfg, model_filename, num_steps):
    add_filename = (
    ...
    base_filename = f'{cfg.model_filename}_{ckpt_filename}_{add_filename}'
    return base_filename


def save_checkpoint(model, optim, step, config, ckpt_dir = '/fsx-maui/dayal/checkpoints/finetuned-gpt'):
    ...
    ckpt_filename = f"{cfg.model_filename}_{get_ckpt_filename(config, step)}.ckpt"
    print(f"saving checkpoint to {os.path.join(ckpt_dir, ckpt_filename)}")
    torch.save(checkpoint, os.path.join(ckpt_dir, ckpt_filename))

    return

def get_last_checkpoint(cfg, device):
    save_steps = [1000*i  for i in range(0, cfg.num_steps//1000 + 1)]

    for step in save_steps[::-1]:
        ckpt_name = f'{cfg.model_filename}_{get_ckpt_filename(cfg, step)}.ckpt'
        ckpt_path = os.path.join(cfg.ckpt_store_dir, ckpt_name)
...
def create_train_state(cfg, device):
    ...
    ckpt_load_path = os.path.join(cfg.ckpt_load_dir, cfg.ckpt_load_path)
    print(f'Loading Finetuning checkpoint from {ckpt_load_path}')
    
    if os.path.exists(ckpt_load_path):
        "Load the checkpoint without the optimizer states"
        ...
        print(f'Finetuning checkpoint not found at {ckpt_load_path}, exiting...')
        exit(0)
...
def train_and_evaluate(cfg, device):
    ...
    ### DATA CONTAINERS ###
    train_results = DataStorage(columns = ['step', 'lr_step', 'loss_step', 'dataset'], num_params = num_params, embd_params = embd_params)
    forward_results = DataStorage(columns = ['step', 'lr_step', 'lr_lower', 'lr_upper', 'num_iters_critical', 'dataset'], num_params = num_params, embd_params = embd_params)
    sharpness_results = DataStorage(columns = ['step', 'lr_step', 'pre_sharpness', 'pre_num_iters', 'sharpness', 'num_iters', 'dataset'], num_params = num_params, embd_params = embd_params)
    eval_results = DataStorage(columns = ['step', 'lr_step', 'train_loss', 'val_loss', 'dataset'], num_params = num_params, embd_params = embd_params)
    ...
    for step in range(init_steps, cfg.num_steps+1):
        ...
        forward_results.add_entry(step = step, lr_step = lr_step, lr_lower = lr_lower, lr_upper = lr_upper, num_iters_critical = num_iters_critical, dataset = cfg.dataset_name)
        ...
        if step % cfg.sharpness_interval == 0 and cfg.master_process:
            # eigenvalue estimation using loppcg method
            pre_sharpness_step, pre_eigvec, pre_num_iters = sharpness_utils.get_pre_sharpness_lobpcg(model, loss_fn, optim, (X, Y), eigvecs = pre_eigvec, tol = cfg.sharpness_tol, max_iters = cfg.max_iters)
            print(f'Pre-sharpness: {pre_sharpness_step:0.4f} computed in {pre_num_iters} steps, critical LR: {crit_thresh/pre_sharpness_step:0.1e}')
            # eigenvalue estimation using loppcg method
            sharpness_step, eigvec, num_iters = sharpness_utils.get_sharpness_lobpcg(model, loss_fn, (X, Y), eigvecs = eigvec, tol = cfg.sharpness_tol, max_iters = cfg.max_iters)
            print(f'Sharpness: {sharpness_step:0.4f} computed in {num_iters} steps')
            
            # For sharpness results
            sharpness_results.add_entry(step = step, lr_step = lr_step, pre_sharpness = pre_sharpness_step, pre_num_iters = pre_num_iters, sharpness = sharpness_step, num_iters = num_iters, dataset = cfg.dataset_name)
        ...
        if step % cfg.ckpt_interval == 0 and cfg.master_process:
            save_checkpoint(model = raw_model, optim = optim, step = step, config = cfg, ckpt_dir = cfg.ckpt_store_dir)
        ...
        if step % cfg.eval_interval == 0 and cfg.master_process:
            ...
            eval_results.add_entry(step = step, lr_step = lr_step, train_loss = losses['train'].item(), val_loss = losses['val'].item(), dataset = cfg.dataset_name)

            print(f"Evaluation on {cfg.dataset_name} at step {step}: train loss: {losses['train']:.4f}, val loss: {losses['val']:.4f}")

            losses = estimate_loss(ctx, model, loss_fn, cfg.eval_steps, os.path.join('data', 'fineweb'), cfg.context_len, cfg.batch_size, device)
            eval_results.add_entry(step = step, lr_step = lr_step, train_loss = losses['train'].item(), val_loss = losses['val'].item(), dataset = 'fineweb')

            print(f"Evaluation on fineweb at step {step}: train loss: {losses['train']:.4f}, val loss: {losses['val']:.4f}")
        ...
        train_results.add_entry(step = step, lr_step = lr_step, loss_step = lossf, dataset = cfg.dataset_name)
...
### model
parser.add_argument('--ckpt_load_dir', type = str, default = '/fsx-maui/dayal/checkpoints/pretrain_gpt')
parser.add_argument('--ckpt_load_path', type = str, required = True)
...
parser.add_argument('--warmup_steps', type = int, default = 500) # number of steps for warmup
...
### evaluation
parser.add_argument('--results_dir', type = str, default = 'finetune_results')
...
parser.add_argument('--max_iters', type = int, default = 200) # tolerance for convergence of the lobpcg method
parser.add_argument('--sharpness_tol', type = float, default = 1e-11) # tolerance for convergence of the lobpcg method
...
parser.add_argument('--ckpt_store_dir', type = str, default = '/fsx-maui/dayal/checkpoints/finetune_gpt')
...
# extract the model properties from the checkpoint path
cfg.model_filename = extract_model_properties(cfg.ckpt_load_path)
base_filename = get_base_filename(cfg, cfg.model_filename, cfg.num_steps)
