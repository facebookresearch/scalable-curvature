...
def train_and_evaluate(cfg, device):
    ...
    train_results = DataStorage(
        columns=["step", "lr_step", "loss_step", "dataset"],
    ...
    eval_results = DataStorage(
        columns=["step", "lr_step", "train_loss", "val_loss", "dataset"],
        num_params=num_params,
        embd_params=embd_params,
    )
    ...
    for step in range(init_steps, cfg.num_steps + 1):
        ...
        if step % cfg.sharpness_interval == 0 and cfg.master_process:
            ...
            pre_sharpness_step, pre_eigvec, pre_num_iters = (
            ...
            print(
                f"Pre-sharpness: {pre_sharpness_step:0.4f} computed in {pre_num_iters} steps, critical LR: {crit_thresh/pre_sharpness_step:0.1e}"
            )
            # eigenvalue estimation using loppcg method
            sharpness_step, eigvec, num_iters = sharpness_utils.get_sharpness_lobpcg(
                model,
                loss_fn,
                (X, Y),
                eigvecs=eigvec,
        ...
        if step % cfg.eval_interval == 0 and cfg.master_process:
            ...
            print(
                f"Evaluation on {cfg.dataset_name} at step {step}: train loss: {losses['train']:.4f}, val loss: {losses['val']:.4f}"
            )
            ...
            print(
                f"Evaluation on fineweb at step {step}: train loss: {losses['train']:.4f}, val loss: {losses['val']:.4f}"
            )
        ...
        train_results.add_entry(
            step=step, lr_step=lr_step, loss_step=lossf, dataset=cfg.dataset_name
        )
...
### model
parser.add_argument("--ckpt_load_dir", type=str, default="./checkpoints")
...
)  # tolerance for convergence of the lobpcg method
parser.add_argument(
...
parser.add_argument("--ckpt_store_dir", type=str, default="./checkpoints")
