...
@torch.no_grad()
def estimate_loss(cfg, ctx, model, loss_fn, eval_steps, data_dir, context_len, batch_size, device):
    out = {}
    model.eval()
    for split in ['train', 'val']:
        losses = torch.zeros(eval_steps)
        for k in range(eval_steps):
            torch.manual_seed(cfg.seed + cfg.seed_offset + k + (1000 if split == 'val' else 0))
...
def save_checkpoint(model, optim, step, config, ckpt_dir = '/checkpoint/maui/shared/dayal/checkpoints/gpt_sharp'):
    """
...
def train_and_evaluate(cfg, device):
    ...
    ### DATA CONTAINERS ###
    if cfg.master_process:
        train_results = DataStorage(columns = ['step', 'lr_step', 'loss_step', 'param_norm', 'grad_norm'], num_params = num_params, embd_params = embd_params)
        ...
        sharpness_results = DataStorage(columns = ['step', 'lr_step', 'pre_sharpness', 'pre_num_iters', 'sharpness', 'num_iters', 'dir_sharpness', 'trace', 'trace_rel_error', 'frob_norm_sq', 'frob_rel_error'], num_params = num_params, embd_params = embd_params)
    ...
    if last_ckpt is not None and cfg.master_process:
        ...
        forward_results_exists = forward_results.load_from_csv(cfg.sharpness_path, num_steps = init_steps) # load previous results if any
        ...
        load_checkpoint = train_data_exists and eval_data_exists and forward_results_exists and sharpness_data_exists     
    ...
    for step in range(init_steps, cfg.num_steps + 1):
        ...
        if cfg.measure_critical_lr and step % cfg.sharpness_interval == 0:
            ...
            (lr_lower, lr_upper), num_iters_critical = critical_lr_utils.compute_critical_learning_rate(
                ctx = ctx, 
                model = model, 
                loss_fn = loss_fn, 
                optim = optim, 
            ...
            print(f'Pre-sharpness: {pre_sharpness_step:0.1e} computed in {num_iters_pre_sharpness} steps, critical LR: {crit_thresh/pre_sharpness_step:0.1e}')
            ...
            print(f'Sharpness: {sharpness_step:0.1e} computed in {num_iters_sharpness} steps')

            optim.zero_grad(set_to_none = True)

            num_iters, trace_step, trace_rel_err, frob_norm_sq_step, frob_norm_rel_err = sharpness_utils.get_trace_hutchinson(model, loss_fn, optim, (X, Y), max_iters = cfg.max_trace_iters, tol = cfg.trace_tol)
            print(f'Trace: {trace_step:0.1e} computed in {num_iters} steps, relative error: {trace_rel_err:0.1e}')
            # print the ratio of pre-conditioned sharpness to trace
            print(f'Ratio of Pre-conditioned Sharpness to Trace: {pre_sharpness_step/trace_step:0.1e}')
            print(f'Ratio of Pre-conditioned Sharpness to Frobenius norm: {pre_sharpness_step/np.sqrt(frob_norm_sq_step):0.1e}')

            optim.zero_grad(set_to_none = True)

            # direction sharpness estimation
            dir_sharpness_step = sharpness_dir_utils.get_dir_sharpness(ctx = ctx, model = model, loss_fn = loss_fn, optim = optim, batch = (X, Y), recompute_grads = cfg.recompute_grads)
            print(f'Direction Sharpness: {dir_sharpness_step:0.4f}, compare with {2/lr_upper:0.4f} for stability')
        ...
        if step % cfg.eval_interval == 0:
            losses = estimate_loss(cfg, ctx, model, loss_fn, cfg.eval_steps, data_dir, cfg.context_len, cfg.batch_size, device)
            if cfg.master_process:
        ...
        for micro_step in range(cfg.gradient_accumulation_steps):
            ...
            torch.manual_seed(cfg.seed + cfg.seed_offset + step * cfg.gradient_accumulation_steps + micro_step + 1)
            X, Y = get_batch(data_dir, 'train', cfg.context_len, cfg.batch_size, device)
            # backward pass
            loss.backward()
        ...
        grad_norm = torch.norm(torch.stack([torch.norm(p.grad.detach(), 2) for p in model.parameters() if p.grad is not None]), 2).item()
        ...
        if cfg.master_process:
            print(f"step: {step}, lr: {lr_step:0.1e}, loss: {lossf:.4f}, grad_norm: {grad_norm:.1e}")
            train_results.add_entry(step = step, lr_step = lr_step, loss_step = lossf, param_norm = param_norm, grad_norm = grad_norm)
...
parser.add_argument('--ckpt_dir', type = str, default = '/checkpoint/maui/shared/dayal/checkpoints/gpt_sharp')
...
parser.add_argument('--measure_critical_lr', type = lambda x: x.lower() == 'true', default = True)
# sharpness estimation
...
parser.add_argument('--max_trace_iters', type = int, default = 40) # tolerance for convergence of the power method
parser.add_argument('--sharpness_tol', type = float, default = 1e-11) # tolerance for convergence of the power method
parser.add_argument('--trace_tol', type = float, default = 1e-5) # tolerance for convergence of the power method
...
if cfg.ddp:
    ...
    cfg.seed_offset = 0
    ddp_world_size = 1
    init_process_group(backend = backend, world_size = 1, rank = 0, init_method = 'file:///tmp/tmpfile') # this is a dummy init method for single process
