...
def train_and_evaluate(cfg, device):
    ...
    for step in range(cfg.num_steps+1):
        ...
        if cfg.estimate_critical_metrics:
            ...
            sharpness_step, eigvec, num_iters_power = sharpness_utils.get_presharpness_power_method(model = model, loss_fn = loss_fn, optim = optim, batch = (X, Y), max_iters = cfg.max_iters, tol = cfg.sharpness_tol, vec = eigvec)
            print(f'Sharpness: {sharpness_step:0.4f} computed in {num_iters_power} steps, critical LR: {(2 + 2*cfg.beta1)/sharpness_step:0.1e}')
            result = np.asarray([step, sharpness_step.item(), num_iters_power, lr_lower, lr_upper, num_iters_critical])
        ...
        if step % cfg.eval_interval == 0 and cfg.master_process:
            ...
            df_forward = pd.DataFrame(forward_results, columns = ['step', 'sharpness', 'num_iters_power', 'lr_lower', 'lr_upper', 'num_iters_critical'])
...
parser.add_argument('--sharpness_tol', type = float, default = 1e-05) # tolerance for convergence of the power method
