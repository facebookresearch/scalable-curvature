...
import utils.sharpness_utils as sharpness_utils 
...
def train_and_evaluate(cfg, device):
    ...
    for step in range(cfg.num_steps+1):
        ...
        if step % cfg.log_interval == 0 and cfg.master_process:
            ...
            # (X, Y) already loaded but are different!
            eigvals = sharpness_utils.get_hessian_batch(ctx = ctx, model = model, loss_fn = loss_fn, batch = (X, Y), topk = cfg.topk, tol = cfg.tol)
            print(f'Sharpness: {eigvals[0]}')
            result = np.column_stack((np.full_like(eigvals, step), eigvals, np.arange(1, len(eigvals)+1)))
            hessian_results.append(result)

            # critical learning rate estimation
            (lr_lower, lr_upper), num_iters = compute_critical_learning_rate(ctx = ctx, model = model, loss_fn = loss_fn, batch = (X, Y), lr_guess = lr_guess, tol_power = cfg.tol_power)
            lr_guess = lr_lower
            print(f'LR Range: {lr_lower:0.6f}, {lr_upper:0.6f} computed in {num_iters} steps')
            result = np.asarray([step, lr_lower, lr_upper, num_iters])
    ...
    # hessian_results = np.concatenate(hessian_results, axis = 0)
    # df_hessian = pd.DataFrame(hessian_results, columns = ['step', 'eigval', 'k'])
    # df_train['num_params'] = num_params
    # df_train['embd_params'] = embd_params
    # df_hessian.to_csv(cfg.hessian_path)

    forward_results = np.asarray(forward_results)
    df_forward = pd.DataFrame(forward_results, columns = ['step', 'lr_lower', 'lr_upper', 'num_iters'])
...
hessian_filename = f'hessian_{base_filename}.csv'
cfg.hessian_path = os.path.join('results', hessian_filename)

forward_filename = f'forward_{base_filename}.csv'
cfg.forward_path = os.path.join('results', forward_filename)
