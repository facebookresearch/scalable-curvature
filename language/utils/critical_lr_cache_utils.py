...
def virtual_step(model: torch.nn.Module, optim: torch.optim.Optimizer, lr: float) -> Dict[str, Tensor]:
    ...
    for group in optim.param_groups:
        for name, param in zip(group['param_names'], group['params']):
            ...
            updates[param_key] = update  # updates already have the negative sign applied         
...
def compute_critical_learning_rate(
    ...
    def update_and_compute_loss(lr: float):
        ...
        updates_dict = virtual_step(model = model, optim = optim, lr = lr) # Delta Theta; optimizer step but without modifying optimizer state
    ...
    def exponential_search(loss_before: float, lr_guess: float, max_iters: int = 100,):
        ...
        while iteration < max_iters:
            ...
            loss_next = update_and_compute_loss(lr)
            if logger:
                logger.info(f'Exponential search iteration: {iteration} Loss before: {loss_before:0.4f} Loss next: {loss_next:0.4f} LR: {lr:0.4e}')
    ...
    def binary_search(loss_before: float, lr_lower: float, lr_upper: float, tol: float = 1/16):
        ...
        while (1 - lr_lower / lr_upper) > tol:
            ...
            if logger:
                logger.info(f'Binary search iteration: {iterations} Loss before: {loss_before:0.4e} Loss mid: {loss_mid:0.4e} LR lower: {lr_lower:0.1e} LR upper: {lr_upper:0.1e} LR mid: {lr_mid:0.1e}')
            iterations += 1
    ...
    if debug:
        ...
        assert np.isclose(loss_final, loss_params), f'Loss mismatch! {loss_final=} {loss_params=}'  
